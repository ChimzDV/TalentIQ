"""
database.py — TalentIQ Staffing
SQLite database helper for Flask backend.
Mirrors the behaviour of database.js exactly:
  - Creates tables with CREATE TABLE IF NOT EXISTS (safe to run on existing DB)
  - Seeds default admin and jobs ONLY when the respective tables are empty
  - Uses bcrypt to hash the admin password (hashes are cross-compatible with bcryptjs)
"""
...
# Path resolution
# ---------------------------------------------------------------------------
# The database lives one directory UP from this file (project root), so that
# both the old Node.js server and the new Flask server share the same file.
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(_THIS_DIR, '..', 'talentiq.db')
...
def get_connection():
    ...
    conn.execute("PRAGMA journal_mode=WAL") # better concurrency under Passenger
    return conn


@contextmanager
def db_connection():
    """Context manager that auto-commits or rolls back on error."""
    conn = get_connection()
    try:
        yield conn
        conn.commit()
...
# ---------------------------------------------------------------------------
# Schema initialisation  (CREATE TABLE IF NOT EXISTS — 100% safe on existing DB)
# ---------------------------------------------------------------------------
def init_db():
...
# ---------------------------------------------------------------------------
# Seed helpers  (only run when tables are EMPTY — mirrors database.js logic)
# ---------------------------------------------------------------------------
def seed_db():
    """Seed default admin and jobs if tables are empty."""
    _seed_admin()
    _seed_jobs()


def _seed_admin():
    """Insert the default admin account only when the admins table is empty."""
    with db_connection() as conn:
        row = conn.execute("SELECT COUNT(*) AS cnt FROM admins").fetchone()
        if row['cnt'] > 0:
            return  # already seeded

        username = os.environ.get('ADMIN_USERNAME', 'admin@talentiq.com')
        password = os.environ.get('ADMIN_PASSWORD', 'TalentIQ2026!')

        # bcrypt hashes are identical across Python (bcrypt) and Node (bcryptjs)
        ...
        print(f"[DB] Default admin created: {username}")


def _seed_jobs():
    """Insert the initial six job listings only when the jobs table is empty."""
    with db_connection() as conn:
        ...
        insert_sql = """
            INSERT INTO jobs (
                title, industry, location, state, employment_type, experience,
                salary, short_description, description, requirements,
                responsibilities, benefits, status
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """

        for job in initial_jobs:
            conn.execute(insert_sql, (
                job['title'], job['industry'], job['location'], job['state'],
                job['employment_type'], job['experience'], job['salary'],
                job['short_description'], job['description'], job['requirements'],
                job['responsibilities'], job['benefits'], job['status'],
            ))

        print("[DB] Initial jobs seeded successfully.")
